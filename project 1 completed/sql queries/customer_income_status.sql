...
create table new_customer_det as select *,
...
case when applicantincome < 5000 and property_area = 'rural' then 3
     when applicantincome < 5000 and property_area = 'semirural' then 3.5
     when applicantincome < 5000 and property_area = 'urban' then 5
     when applicantincome < 5000 and property_area = 'semiurban' then 2.5 else 7 end as Interest_Rate
from customer_income;

select * from new_customer_det;

select * from loan_status;
select * from remarks_updating_detials;
alter table loan_status rename column `Cibil Score` to Cibil_Score;
alter table loan_status modify loanamount varchar(50);
alter table remarks_updating_detials modify loanamount varchar(50);
truncate loan_status;
truncate remarks_updating_detials;

create table remarks_updating_detials (loan_id varchar(20),
...
delimiter //
create trigger update_null before insert on loan_status for each row
begin
if new.loanamount is null then set new.loanamount = 'Loan_Still_Processing';
end if;
end //
delimiter ;

drop trigger update_null;

delimiter //
create trigger update_cibil_status after insert on loan_status for each row
begin

if new.cibil_score > 900 then 
   insert into remarks_updating_detials (loan_id, customer_id, loanamount, loan_amount_term, cibil_score, cibil_score_status)
   value (new.loan_id, new.customer_id, new.loanamount, new.loan_amount_term, new.cibil_score, 'High_Cibil_Score');

elseif new.cibil_score > 750 then 
   insert into remarks_updating_detials (loan_id, customer_id, loanamount, loan_amount_term, cibil_score, cibil_score_status)
   value (new.loan_id, new.customer_id, new.loanamount, new.loan_amount_term, new.cibil_score, 'No_Penalty');

elseif new.cibil_score >= 0 then 
   insert into remarks_updating_detials (loan_id, customer_id, loanamount, loan_amount_term, cibil_score, cibil_score_status)
   value (new.loan_id, new.customer_id, new.loanamount, new.loan_amount_term, new.cibil_score, 'Penalty_Customer');

else
   insert into remarks_updating_detials (loan_id, customer_id, loanamount, loan_amount_term, cibil_score, cibil_score_status)
   value (new.loan_id, new.customer_id, new.loanamount, new.loan_amount_term, new.cibil_score, 'Rejected_Customer');
...
drop trigger update_cibil_status;

insert into loan_status (loan_id, customer_id, loanamount, loan_amount_term, cibil_score) 
values 
...
-- delete from remarks_updating_detials where loanamount = 'loan_still_processing' or cibil_score_status = 'Rejected_Customer';

select * from remarks_updating_detials;
alter table remarks_updating_detials modify loanamount int;
select * from new_customer_det;
alter table new_customer_det rename column `Customer ID` to Customer_ID;

create table updated_customer_and_loan_table as
select n.*, r.loanamount, r.loan_amount_term, r.cibil_score, r.cibil_score_status from new_customer_det n 
inner join remarks_updating_detials r on n.customer_id = r.customer_id;

drop table customer_interest_analysis;

create table customer_interest_analysis as
select *,
round((((loanamount * interest_rate) / 12 )/ 100)) as monthly_interest_amount,
round(((loanamount * interest_rate) / 100)) as annual_interest_amount
from updated_customer_and_loan_table;

alter table customer_interest_analysis modify Interest_Rate varchar(10);

update customer_interest_analysis set Interest_Rate = concat(Interest_Rate, '%');
